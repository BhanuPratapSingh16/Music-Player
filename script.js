...
function displaySong(songs) {
    songs.forEach((element, index) => {
        ...
        songitem.innerHTML = `<span>${modifySongName(element.songname)}</span> <span class="play-this" id="${index}"> ${element.duration} <i class="fa-solid fa-play"></i> </span>`;
...
function modifySongName(songname){
    let newsongname = ""
    for(let i=0;i<songname.length-4;i++){
        if(songname.charAt(i) == '-')
            newsongname = newsongname + ' ';
        else
            newsongname = newsongname + songname.charAt(i);
    }
    return newsongname
}
